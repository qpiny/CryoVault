...
(defn mainXX []
  (widget :node "kk" :node "ll")
  ;(node [:div "plop"])
  ;(aa (str ":n" "ode1") :node2)
  )
;  (widget :split "vertical"
;           (widget :split "horizontal"
;                    (widget :zip
;                             (widget :node [:h2.zip-header "Inventory"])
;                             (widget :node [:div
;                                             [:div "Date "   [:span#inv-date "unknown"]]
;                                             [:div "Status " [:span#inv-status "unknown"] [:div#inv-refresh]]
;                                             [:div#snapshot-list]]))
;                    (widget :node [:div "coucou2"]))
;           (widget :node [:div "logs"])))
