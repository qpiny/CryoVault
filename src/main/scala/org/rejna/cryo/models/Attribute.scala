...
//object EventPath extends Regex("/cryo/([^/]*)/([^/]*)#(.*)", "service", "object", "attribute")
//case class EventPath(path: List[String], attributeName: String) {
//  val classifier = path.mkString(".") + '#' + attributeName
//}

case class AttributeChange[A](path: EventPath, previous: Option[A], now: A) extends Event
case class AttributeListChange[A](path: EventPath, addedValues: List[A], removedValues: List[A]) extends Event

trait AttributeSimpleCallback {
  def onChange[A](name: EventPath, previous: Option[A], now: A)
}

trait AttributeListCallback {
  def onListChange[A](name: EventPath, addedValues: List[A], removedValues: List[A])
}

object Attribute {
  def apply[A](name: EventPath, initValue: A)(implicit cryoctx: CryoContext): SimpleAttribute[A] =
    new SimpleAttribute(name)(None, initValue)

  def apply[A](name: EventPath, body: () => A)(implicit cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration) =
    new MetaAttribute(name, () => Future(body()))

  def future[A](name: EventPath, body: () => Future[A])(implicit cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration) =
    new MetaAttribute(name, body)

  def list[A](name: EventPath, initValue: List[A])(implicit cryoctx: CryoContext): ListAttribute[A] =
    new ListAttribute[A](name)(None, initValue)

  def list[A](name: EventPath, body: () => List[A])(implicit cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration) =
    new MetaListAttribute(name, () => Future(body()))

  def futureList[A](name: EventPath, body: () => Future[List[A]])(implicit cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration) =
    new MetaListAttribute(name, body)

  def map[A, B](name: EventPath, initValue: IMap[A, B])(implicit cryoctx: CryoContext): MapAttribute[A, B] =
    new MapAttribute[A, B](name)(None, initValue.toList)

  def map[A, B](name: EventPath, body: () => IMap[A, B])(implicit cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration) =
    new MetaMapAttribute(name, () => Future(body()))

  def futureMap[A, B](name: EventPath, body: () => Future[IMap[A, B]])(implicit cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration) =
...
trait SimpleCallbackable extends LoggingClass { self: Attribute[_] =>
  ...
  def onChange(cb: (=> Unit) => Unit) = {
    lazy val ac: AttributeSimpleCallback = new AttributeSimpleCallback {
      override def onChange[B](name: EventPath, previous: Option[B], now: B): Unit = cb { removeCallback(ac) }
    }
    addCallback(ac)
  }
...
class MetaAttribute[A](val name: EventPath, body: () => Future[A])(implicit val cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration)
  ...
  object Invalidate extends AttributeSimpleCallback {
    override def onChange[C](name: EventPath, previous: Option[C], now: C) = invalidate
  }
...
class MetaListAttribute[A](name: EventPath, body: () => Future[List[A]])(implicit cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration)
  extends MetaAttribute[List[A]](name, body)(cryoctx, executionContext, timeout)
  with ListCallbackable[A]

class MetaMapAttribute[A, B](name: EventPath, body: () => Future[IMap[A, B]])(implicit cryoctx: CryoContext, executionContext: ExecutionContext, timeout: Duration)
  extends MetaAttribute[List[(A, B)]](name, () => body().map(_.toList))(cryoctx, executionContext, timeout)
  with ListCallbackable[(A, B)]

class SimpleAttribute[A](val name: EventPath)(private var _previous: Option[A], var _now: A)(implicit val cryoctx: CryoContext) extends Attribute[A] with SimpleCallbackable {
...
trait ListCallbackable[A] { self: Attribute[List[A]] with SimpleCallbackable =>
  ...
  addCallback(new AttributeSimpleCallback {
    override def onChange[C](name: EventPath, _previous: Option[C], _now: C): Unit = {
...
class ListAttribute[A](name: EventPath)(__previous: Option[List[A]], __now: List[A])(implicit cryoctx: CryoContext)
...
class MapAttribute[A, B](name: EventPath)(__previous: Option[List[(A, B)]], __now: List[(A, B)])(implicit cryoctx: CryoContext)
