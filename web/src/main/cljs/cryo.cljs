...
(defn- websocket-opened [soc]
  (fn [msg] (ws/emit! soc "subscribe" {:subscription "/cryo/inventory"})))
(defn- websocket-message [soc msg]
  (logger/log "INFO" "ws-r" msg))
(defn- websocket-error [soc msg]
  (logger/log "ERROR" "ws-e" msg))
(defn- websocket-closed [soc msg]
  (logger/log "INFO" "ws-c" msg))
...
(ws/configure
  (str "ws://" (-> js/document .-location .-host) + "/websocket/") 
  websocket-opened
  websocket-message
  websocket-error
  websocket-closed)
